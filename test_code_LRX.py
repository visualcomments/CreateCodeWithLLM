...
# =============================================================================
# === CONFIG: PROMPTS UPDATED FOR CONSTRUCTIVE SORTING TASK ===
# =============================================================================
...
CONFIG = {
    ...
    'PROMPTS': {
        # This is the NEW task (Prompt 2) - As requested by user
        'INITIAL': r"""
...
- Implement a function solve(vector) that returns a tuple (moves, sorted_array):
    ...
    - sorted_array: the final sorted array after applying all moves
...
""",
        # NEW FIX prompt - adapted to new constraints
        'FIX': r"""
...
• The sorting logic must use *only* L, R, X operations internally.
• CRITICAL: The algorithm MUST be polynomial-time (O(n^k)).
• CRITICAL: DO NOT use BFS, DFS, or any exponential-time graph search. Use a constructive, iterative approach.
• Self-contained, executable, immediately correct.
• Only code in response, no extra prints or Markdown.
""",
        # NEW REFACTOR prompt - adapted to new constraints
        'REFACTOR_NO_PREV': r"""
...
⚠️ Goals:
• Improve the sorting algorithm's efficiency.
• Ensure it's a *constructive, polynomial-time* algorithm (e.g., O(n^2), O(n^3)).
• DO NOT use BFS, DFS, or any exponential-time graph search.
...
"""
    ...
    'CONSTANTS': {
        ...
        'N_SAVE': 100,
        'MAX_WORKERS': 10,
        'EXEC_TIMEOUT': 10, # 10 seconds is enough for poly-time, too short for N!
        'ERROR_TIMEOUT': 'Timeout expired — the sorting algorithm is too slow (likely not polynomial-time) or stuck in an infinite loop.',
...
def test_code(code: str, config: Dict) -> Tuple[bool, str, Optional[Dict]]:
    ...
    # We use smaller N because sorting (even poly-time) can be slow
    specific_vectors = [
        [],                      # n=0
        [1],                     # n=1
        ...
        [1, 5, 2, 4, 3],         # n=5
        [6, 1, 2, 3, 4, 5]       # n=6 (testing timeout for slow poly/bad loops)
    ]
    # ---
    
    # We remove the random loop for n=4-20 as it's not needed
    # for this type of constructive problem test.
